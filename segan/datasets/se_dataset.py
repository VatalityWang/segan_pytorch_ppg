...
def slice_signal_index(path, window_size, stride):
    ...
    """ Slice input signal into indexes (beg, end) each
        ...
        # Arguments
            ...
            stride: fraction of sliding window per window size  步长 每一个滑动窗口的大小

        # Returns
            A list of tuples (beg, end) sample indexes
    """
    #pdb.set_trace()
    # signal, rate = librosa.load(path, 16000)#以16khz的采样率读取音频文件  缺省采样率为22050  返回时间序列和采样率
    ppgsignal=[]
    ...
    assert signal.ndim == 1, signal.ndim
    n_samples = signal.shape[0]#总样本点数
    #print("n_samples=%d,type(signal) %s"%(n_samples,type(signal)))
    slices = []
    #offset = 2#步长偏移
    offset = int(window_size * stride)  # 步长偏移
...
class PPGDataset(Dataset):
    ...
    def __init__(self,clean_dir,noisy_dir):
        ...
        self.clean_names = glob.glob(os.path.join(clean_dir, '*.mat'))  # 查找符合特定规则的文件名 路径
        self.noisy_names = glob.glob(os.path.join(noisy_dir, '*.mat'))
        ...
        if len(self.clean_names) != len(self.noisy_names) or \
            ...
            raise ValueError('No ppg data found! Check your data path please')


    def read_excel_file(self,excelfilname):
        ppgsignal = []
        # ipdb.set_trace()
        ppg=np.zeros(200)
        if(excelfilname[-3:]=='mat'):
            # print('filename: ', excelfilname)
            signal=loadmat(excelfilname)
            # print('signal shape:',signal.shape)
            listname=excelfilname.split('/')
            # print('listname:',listname)

            ##############################################################
            # change the dir have to change the code to adapte the dictory
            ##############################################################

            if listname[1]=='clean_trainset' or listname[1]=='clean_valset' or listname[2]=='Time_clean_10':
                signal=signal['HandlePPG']
            else:
                signal=signal['ytmp1']
            # signal=signal['']
            signal=signal.squeeze() # 压缩为1的维度
            #ppg[0:200]=signal[:]
            #return ppg
            return signal
    ...
    def __getitem__(self, index):
        ...
        bname = os.path.splitext(os.path.basename(self.clean_names[index]))[0]
        c_slice=self.read_excel_file(self.clean_names[index])
        n_slice=self.read_excel_file(self.noisy_names[index])
        #print('c_name:{},n_name:{}'.format(self.clean_names[index],self.noisy_names[index]))
        slice_idx=index
        listslice=self.clean_names[index].split('-')
        # ipdb.set_trace()
        label=listslice[2][:-4]
        # print(type(label))
        ...
        labels=np.zeros(200)
        labels[labelInt-1]=1
        # baname=self.clean_names[index],self.noisy_names[index]
...
class SEDataset(Dataset):
    ...
    def __init__(self, clean_dir, noisy_dir, preemph, cache_dir='.', 
                 split='train', slice_size=872,
                 stride = 0.125, max_samples=None, do_cache=False, verbose=False,
                 slice_workers=2, preemph_norm=False,
                 random_scale=[1]):
        super(SEDataset, self).__init__() #调用分类的初始化方法
        print('Creating {} split out of data in {}'.format(split, clean_dir))
        self.clean_names = glob.glob(os.path.join(clean_dir, '*.xlsx'))#查找符合特定规则的文件名 路径
        self.noisy_names = glob.glob(os.path.join(noisy_dir, '*.xlsx'))
        # pdb.set_trace()
        print('Found {} clean names and {} noisy'
              ' names'.format(len(self.clean_names), len(self.noisy_names)))
    ...
    def read_excel_file(self,excelfilname):
        ...
        ''''
        if self.preemph_norm:
            # signal = pre_emphasize(signal, self.preemph)
            signal = normalize_minmax(signal)
        else:
            signal = normalize_minmax(signal)
            # signal = pre_emphasize(signal, self.preemph)
        '''
    ...
    def write_to_excel(self,out_path,ppg):
        wb = Workbook()
    ...
    def extract_slice(self, index):
        ...
        with open(slice_file, 'rb') as s_f:
            ...
            c_signal = self.read_excel_file(slice_['c_path'])[1]
            n_signal = self.read_excel_file(slice_['n_path'])[1]
...
if __name__ == '__main__':
    # dset = SEDataset('../../data_veu4/expanded_segan1_additive/clean_trainset', '../../data_veu4/expanded_segan1_additive/noisy_trainset', 0.95,
    #                   cache_dir='.', max_samples=100, verbose=True)
    # dset=PPGDataset('../../NewData/clean_trainset', '../../NewData/noisy_trainset')
    dset=PPGDataset('../../NewData/Time_clean_10', '../../NewData/Time_noisy_10')
    # dset=PPGDataset('../../data/clean_trainset', '../../data/noisy_trainset')
    dset.__getitem__(4)
