    ...
    def __init__(self, skip_type, size, skip_init, skip_dropout=0,
        ...
        if skip_type == 'alpha' or skip_type == 'constant':
            if skip_init == 'zero':
                ...
                alpha_ = torch.ones(size)#返回一个全为1的张量
            else:
...
class Generator(Model):
    ...
    def __init__(self, ninputs, fmaps,
        ...
        assert isinstance(fmaps, list), type(fmaps)  #条件为假，终止
        ...
        skips = {}
        ninp = ninputs  #输入数据维数
        self.lablefc=nn.Linear(self.labels+1000,256)
        self.labelact=nn.ReLU(256)
        ...
        if dec_fmaps is None:
            dec_fmaps = fmaps[::-1][1:] + [1] #所有元素逆序之后取第一个及以后的元素(步长为-1，从后往前取元素)，再在列表末尾加上一个元素1
        else:
    ...
    def forward(self, x, z=None,labels=None, ret_hid=False):
        #ipdb.set_trace()    
        hall = {}
        hi = x
        skips = self.skips
...
if __name__ == '__main__':
    ...
    G = Generator1D(1, [64, 128, 256, 512, 1024], 
                    ...
                    dec_kwidth=31)
    """
    G = Generator(1, [32,64,128,256,512],
                  kwidth=4,
                  poolings=[2, 2, 2,2,2], no_z=True,labels=200)
    print(G)
    print('G num params: ', G.get_n_params())
    x = torch.randn(1, 1,1000)
    labels=torch.randint(40,200,(1,200))
    labels=labels.float()
    y, hall = G(x, labels=labels,ret_hid=True)
    print(x)
    print(y)
    print(x.size())
    print(y.size())
