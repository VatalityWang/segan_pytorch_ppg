...
class SEGAN(Model):
    ...
    def generate(self, inwav, z = None,labelhr=None, device='cpu'):
        #ipdb.set_trace()
        self.G.eval()#模型评估阶段
        N = 1000
        x = np.zeros((1, 1, N))
        c_res = None
        #inwav=preprocessing.scale(inwav)
        slice_idx = torch.zeros(1)
        for beg_i in range(0, inwav.shape[2]-N, int(N/4)):
            ...
            if pad  > 0:
                ...
                if isinstance(x[0,0],np.ndarray):
                    ...
                    tmpdata=x[0,0].cpu().numpy()
                    x[0,0]=torch.FloatTensor(preprocessing.scale(tmpdata))
            #x = torch.FloatTensor(x)
            if isinstance(x, np.ndarray):
                x = torch.FloatTensor(x)
            x = x.to(device)
            #print('slice_idx:{}'.format(slice_idx))
            label=labelhr[0][int(slice_idx.cpu().numpy()[0])]
            labelInt=int(float(label)+0.5)
            labels=np.zeros(200)
            labels[labelInt-1]=1
            
            # labelhr=np.array([labelhr])
            # labelhr=labelhr.view(-1,labelhr.size(0))
            # labelhr=torch.FloatTensor(labelhr)
            # labelhr=labelhr.to(device)
            labels=labels.reshape(1,-1)
            if isinstance(labels, np.ndarray):
                labels = torch.FloatTensor(labels)
            labels=labels.to(device)
            canvas_w, hall = self.infer_G(x, z=z,labels=labels, ret_hid=True)
            nums = []
            for k in hall.keys():
            ...
            if c_res is None:
                ...
                c_res = np.concatenate((c_res, canvas_w[750:1000]))#数组拼接函数
            slice_idx += 1
        # de-emph
        #ipdb.set_trace()
    ...
    def infer_G(self, nwav, cwav=None, z=None,labels=None,ret_hid=False):
        if ret_hid:
            Genh, hall = self.G(nwav, z=z, labels=labels,ret_hid=ret_hid)
            return Genh, hall
        else:
            Genh = self.G(nwav, z=z, labels=labels,ret_hid=ret_hid)
            return Genh

    def infer_D(self, x_, ref,labels=None):
        D_in = torch.cat((x_, ref), dim=1)#在行上拼接起来
        return self.D(D_in,labels)

                        
    def gen_train_samples(self, clean_samples, noisy_samples, z_sample, 
                          iteration=None,vis=None,labels=None,batch=None,epoch=None,lendloader=None):
        # pdb.set_trace()
        if z_sample is not None:
            canvas_w = self.infer_G(noisy_samples, clean_samples, z=z_sample,labels=labels)
        else:
            canvas_w = self.infer_G(noisy_samples, clean_samples,labels=labels)
        ...
        for m in range(noisy_samples.size(0)):
            ...
            # visual ppg signal
            ####################
            # viz = Visdom(env='real-ppg')
            ##可视化前5个样本 左边 生成；中 干净；右 带噪
            if m<5:
                f1=f.add_subplot(3,5,m+1)
                f1.plot(m_canvas)

                f2=f.add_subplot(3,5,5+m+1)
                f2.plot(clean_samples[m,0].cpu().data.numpy())

                f3=f.add_subplot(3,5,2*5+m+1)
                f3.plot(noisy_samples[m,0].cpu().data.numpy())
                if m+1==5:
                    # vis.matplot(f)
                    f.savefig(os.path.join(self.save_path,'sample_{}_{}.png'.format(epoch,iteration)))
                    # f.close()

            #############
            # hr---mae
            #############
            arraylabel=labels[m].cpu().numpy()
            indexlabel=np.where(arraylabel==max(arraylabel))
            hr=self.calucate_hr(m_canvas)
            difference.append(abs(hr-(indexlabel[0][0])+1))

            m_canvas = pd.DataFrame(m_canvas)#将array转化成DataFrame
            m_canvas.to_excel(os.path.join(self.save_path,
                                       'sample_{}-'
                                       '{}.xlsx'.format(iteration,m+1)),
                                        header=None,index=None)#写为excel文件。
            # vis.plot('real-ppg',m_canvas)
            ...
            m_gtruth_path = os.path.join(self.save_path,
                                         'gtruth_{}.xlsx'.format(m))
            if not os.path.exists(m_gtruth_path):
                m_clean = pd.DataFrame(m_clean)#将array转化成DataFrame
                m_canvas.to_excel(os.path.join(self.save_path,
                                       'gtruth_{}.xlsx'.format(m+1)),
                                        header=None,index=None)#写为excel文件。
                m_noisy = pd.DataFrame(m_noisy)#将array转化成DataFrame
                m_noisy.to_excel(os.path.join(self.save_path,
                                       'noisy_{}.xlsx'.format(m+1)),
                                        header=None,index=None)#写为excel文件。
                m_dif = pd.DataFrame(m_dif)#将array转化成DataFrame
                m_dif.to_excel(os.path.join(self.save_path,
                                       'dif_{}.xlsx'.format(m+1)),
                                        header=None,index=None)#写为excel文件。
                # wavfile.write(os.path.join(self.save_path,
        ...
        log = '(Iter {}) Batch {}/{} (Epoch {}) MAE:{:.4f} '.format(iteration, batch,
                                                   lendloader, epoch,np.mean(d)
                                                   )
        print(log)
    ...
    def train(self, opts, dloader, criterion, l1_init, l1_dec_step,
        ...
        for epoch in range(1, opts.epoch + 1):
            ...
            for bidx, batch in enumerate(dloader, start=1):
                ...
                sample = batch
                if len(sample) == 5:
                    uttname, clean, noisy, slice_idx,HRlabel = batch
                else:
                ...
                HRlabel=HRlabel.to(device)
                HRlabel=HRlabel.float()
                if noisy_samples is None:
                    noisy_samples = noisy[:20, :, :].contiguous()
                    clean_samples = clean[:20, :, :].contiguous()
                    label_samples = HRlabel[:20,:].contiguous()
                # (1) D real update
                Dopt.zero_grad()
                total_d_fake_loss = 0
                total_d_real_loss = 0
                #ipdb.set_trace()
                Genh = self.infer_G(noisy, clean,labels=HRlabel)#生成器给原始带噪声信号加入高斯噪声--->高斯噪声信号
                lab = label
                d_real, _ = self.infer_D(clean, noisy,labels=HRlabel)#原始噪声信号和干净信号一起加入判别器
                d_real_loss = criterion(d_real.view(-1), lab)#计算原始噪声和干净信号经过判别器的结果和标签之间的损失
                d_real_loss.backward()#真实的损失
                total_d_real_loss += d_real_loss
                
                # (2) D fake update
                d_fake, _ = self.infer_D(Genh.detach(), noisy,labels=HRlabel)#加入生成器输出的高斯噪声信号和原始噪声信号 停止前向传播
                lab = label.fill_(0)
                d_fake_loss = criterion(d_fake.view(-1), lab)
                d_fake_loss.backward()
                total_d_fake_loss += d_fake_loss#计算生成器生成高斯噪声信号和原始噪声信号的损失
                Dopt.step()
                ...
                Gopt.zero_grad()
                lab = label.fill_(1)
                d_fake_, _ = self.infer_D(Genh, noisy,labels=HRlabel)
                ...
                if bidx % log_freq == 0 or bidx >= len(dloader):
                    ...
                    if not opts.no_train_gen and  epoch % opts.plotfreq==0:
                    # if not opts.no_train_gen and bidx % opts.plotfreq==0:
                        #canvas_w = self.G(noisy_samples, z=z_sample)
                        '''
                        self.gen_train_samples(clean_samples, noisy_samples,
                                               z_sample,
                                               iteration=iteration,vis=vis,labels=label_samples,
                                               batch=bidx,epoch=epoch,lendloader=len(dloader))
                        '''
                        # ipdb.set_trace()
                        self.gen_train_samples_onval(va_dloader,epoch=epoch,iteration=iteration,Batch=batch,device=device,vis=vis,length=noisy.shape[2])
                iteration += 1
            '''
        ...
        with torch.no_grad():
            ...
            if do_noisy:
                ...
                for eval_ in map_ret:
                    fill_ret_dict(evals, eval_)
                return evals
    def gen_train_samples_onval(self,va_dloader=None,epoch=None,iteration=None,Batch=None,device=None,vis=None,length=None):
        difference=[]
        nannum=0 
        figx=np.linspace(0,length,length)
        if epoch==1:
            RandomData=np.random.randn(3,length)
            vis.line(RandomData[0],figx,win='noise',name='1',opts={'title':'noisy','xlabel':'time','ylabel':'signal'})
            vis.line(RandomData[1],figx,win='clean',name='2',opts={'title':'clean','xlabel':'time','ylabel':'signal'})
            vis.line(RandomData[2],figx,win='denoise',name='3',opts={'title':'denoise','xlabel':'time','ylabel':'signal'})
        #ipdb.set_trace()
        for bidx, batch in enumerate(va_dloader, start=1):
            sample = batch
            if len(sample) == 5:
                    uttname, clean, noisy, slice_idx,HRlabel = batch
            else:
            ...
            HRlabel=HRlabel.float()
            num=clean.size(0)
            labels=HRlabel[:num,:].contiguous()
            ##############################
            # generate z in evaluaton
            ##############################
            #ipdb.set_trace()
            if not hasattr(self.G, 'z'):
                z_sample=None
            else:
                z_sample = self.G.z[:num, :, :].contiguous()
                z_sample = z_sample.to(device)
            
           
            if z_sample is not None:
                canvas_w = self.infer_G(noisy, clean, z=z_sample,labels=labels)
            else:
                canvas_w = self.infer_G(noisy, clean,labels=labels)
            sample_dif = noisy - clean
            ...
            '''
            #ipdb.set_trace()
            for m in range(noisy.size(0)):
                #m_canvas = de_emphasize(canvas_w[m,
                #                          0].cpu().data.numpy(),
                #                  self.preemph)
                m_canvas=canvas_w[m,0].cpu().data.numpy()
                ####################
                # visual ppg signal
                ####################
                if bidx==1 and m==4:
                    vis.line(noisy[m,0].cpu().data.numpy(),figx,update='replace',win='noise',name='1')
                    vis.line(clean[m,0].cpu().data.numpy(),figx,update='replace',win='clean',name='2')
                    vis.line(m_canvas,figx,update='replace',win='denoise',name='3')
                '''
                if m<5:
                    f1=f.add_subplot(3,5,m+1)
                    f1.plot(m_canvas)
                    ...
                    f2=f.add_subplot(3,5,5+m+1)
                    f2.plot(clean[m,0].cpu().data.numpy())

                    f3=f.add_subplot(3,5,2*5+m+1)
                    f3.plot(noisy[m,0].cpu().data.numpy())
                if m+1==5:
                    # vis.matplot(f)
                    f.savefig(os.path.join(self.save_path,'sample_{}_{}.png'.format(epoch,iteration)))
                    # f.close()
        ...
        log = '(Iter {}) Batch {}/{} (Epoch {}) MAE:{:.4f} nannum:{} '.format(iteration, bidx,
                                                len(va_dloader), epoch,np.mean(d),    nannum)
        print(log)    
    ################
    # 实时计算心率
    ###############
    def Loc2BPM(self,loc, srate, Nfft):
        BPM = (loc-1)/Nfft * srate * 60
        return BPM
    def calucate_hr(self,signal):
        Nfft=4096
        srate=125   
        # HRrange=np.arange(round(1*Nfft/srate),round(4*Nfft/srate)+2)
        # psd=1/len(signal)*fft(signal,Nfft)**2
        wd, m = hp.process(signal, sample_rate = srate,bpmmax=65535)
...
class WSEGAN(SEGAN):
    ...
    def generate(self, inwav, z = None):
        ...
        p_wav = make_divN(inwav.transpose(1, 2), 512).transpose(1, 2)
