...
from segan.datasets import SEDataset, SEH5Dataset, collate_fn,PPGDataset
...
def main(opts):
    ...
    if opts.h5:
        ...
        ########################
        # handle excel dataset
        #########################
        if opts.ppgantype=='ppgan_time':
            ...
            #print("SEDatasetMaxNum",dset.getmaxnum())
        else:
            dset=PPGDataset(opts.clean_trainset,opts.noisy_trainset)
   
    # spilt the dataset   
    if opts.val_spilt is not None:
        #ipdb.set_trace()
        train_size=int(opts.val_spilt*len(dset))
        test_size=len(dset)-train_size
        trainset,testset= torch.utils.data.random_split(dset,[train_size,test_size])
        print('trainsize:{},testsize:{}'.format(train_size,test_size))
        dloader= DataLoader(trainset, batch_size=opts.batch_size,
                         shuffle=True, num_workers=opts.num_workers,
                         pin_memory=CUDA,
                         collate_fn=collate_fn)
        va_dloader= DataLoader(testset, batch_size=opts.batch_size,
    ...
    if opts.clean_valset is not None:
        if opts.h5:
            ...
            va_dset = SEDataset(opts.clean_valset, 
                                ...
                                preemph_norm=opts.preemph_norm)
            '''
            va_dset=PPGDataset(opts.clean_valset,opts.noisy_valset)
        # pdb.set_trace()
    ...
    elif opts.d_pretrained_ckpt and opts.g_pretrained_ckpt:
        segan.gen_train_samples_onval(va_dloader,epoch=0,iteration=0,Batch=0,device=device)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--save_path', type=str, default="seganv1_ckpt",
                        help="Path to save models (Def: seganv1_ckpt).")
    # parser.add_argument('--d_pretrained_ckpt', type=str, default='ppgan_time/model0504/epoch500z_512_01/weights_EOE_D-Discriminator-16501.ckpt',
    #                     help='Path to ckpt file to pre-load in training '
    #                          '(Def: None).')
    # parser.add_argument('--g_pretrained_ckpt', type=str, default='ppgan_time/model0504/epoch500z_512_01/weights_EOE_G-Generator-16501.ckpt',
    #                     help='Path to ckpt file to pre-load in training '
    #                          '(Def: None).')
    ...
    parser.add_argument('--clean_trainset', type=str,
                        default='data/clean_trainset_HR')
    parser.add_argument('--noisy_trainset', type=str,
                        default='data/noisy_trainset_HR')
    ...
    parser.add_argument('--epoch', type=int, default=200) #所有训练样本的正向传递和反向传递
    ...
    parser.add_argument('--gkwidth', type=int, default=4)
    ...
    parser.add_argument('--ppgantype', type=str, default='ppgan_time')
    parser.add_argument('--labels',type=int,default=200)
    parser.add_argument('--vis',type=bool,default=True)
    parser.add_argument('--env',type=str,default='PPGVIS')
    parser.add_argument('--plotfreq',type=int,default=1)
    parser.add_argument('--stage',type=str,default='train')
    parser.add_argument('--val_spilt',type=float,default=None)
